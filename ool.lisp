...
unable to create the class -> ~a is not an atom~%" class-name)))
        ((equal class-name '())
         (error (format nil "Def-class: ~
unable to create the class -> ~a is an empty list~%" class-name)))
        ((null class-name)
         (error (format nil "Def-class: ~
...
unable to create the class -> ~a is not a list~%" 
        ...
        ((is-class class-name)
         (error (format nil "Def-class: ~
unable to create the class -> ~a already exists~%" class-name)))
        ((and parents
...
(defun make (class-name &rest fields)
  (cond ((null class-name)
	 (error (format nil "Make: ~
class-name is NULL~%")))
	((not (is-class class-name))
	 (error (format nil "Make: ~
class ~a not found~%" class-name)))
...
;;;istanza -> instance è il nome dell'istanza
  (cond ((not (is-class instance))
	 (cond ((cond ((or (atom instance)
		       ...
		       (error 
			(format nil "Field: ~
~a is not a class or an instance~%" instance)))
...
Checking ~a in instance ~a~%" field-name instance)
		(cond ((null field-name)
		       (error (format nil "Field (instance): ~
field-name is NULL")))
		      ((listp field-name)
		       (error (format nil "Field (instance): ~
field-name is a list")))		      
		      (T (cond ((not (member field-name
					     (get-fields-name-of-class
					      (nth 2 instance))))
				(error (format nil "Field (instance): ~
field ~a does not exist in the instance ~a~%" field-name instance)))
...
Checking ~a in class of ~a~%" method-name class-name)
        (cond ((or
                (null 
...
Checking in parents...~%"  method-name (nth 1 class-name))
               (cond 
                 (	
                  (null (cadr (get-class-spec 
                               (nth 1 class-name))))
                  (error 
                   (format nil "Is-method: ~
this class has no parents. Method ~a does not exist!~%" method-name)))
    ...
    (T 
     (format *STANDARD-OUTPUT* 
             "Is-method (class): ~
Checking ~a in class ~a~%" method-name class-name)
     (cond (
            (not 
             (member 
              method-name 
            ...
            (format *STANDARD-OUTPUT* "Is-method: ~
method ~a not found in this class (~a). ~
Checking in parents...~%" method-name class-name)
            (cond (
                   (null (cadr (get-class-spec class-name)))
                   (error 
                    (format nil "Is-method: ~
Class has no parents. Method ~a does not exist!~%"  method-name)))
                  (T  
                   (get-parents-method 
                    (cadr 
                     (get-class-spec class-name)) 
