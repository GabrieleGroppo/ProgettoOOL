	...
	(let
		...
		(ecase value
			...
			(1 (cond ((and (not (equal (caar parts) 'fields))
				      ...
				      (error (format nil "Parts-structure: invalid fields or methods spelling~%")))
...
;;;istanza -> instance è il nome dell'istanza
  (cond ((not (is-class instance))
	 (cond ((cond ((or (atom instance)
		       ...
		       (error 
				 	(format nil 
						"Field: ~a is not a class or an instance~%" 
						instance)))
		...
		(format *STANDARD-OUTPUT* 
			"Field (instance): Checking ~a in instance ~a~%" 
				field-name 
				instance)
		(cond 	(	(null field-name)
					(error (format nil "Field (instance): field-name is NULL")))
		
				(	(listp field-name)
					(error (format nil "Field (instance): field-name is a list")))
				...
				(T (cond (	(not 
					(member 
						...
						(get-fields-name-of-class (nth 2 instance))))
		      			(error 
							(format nil 
								"Field (instance): field ~a does not exist in the instance ~a~%" 
							field-name 
							instance)))
		     			(t  (format *STANDARD-OUTPUT* 
								"Field (instance): field ~a found in this instance (~a)~%"
			  				field-name 
							(get-fields-name-of-class (nth 2 instance))))))))))
;;;classe -> instance è il nome della classe
	(t (format *STANDARD-OUTPUT* "Field (class): Checking ~a in class ~a~%" 
	   ...
	   (cond ((not 
		  ...
		  (format *STANDARD-OUTPUT* 
		  	"field (class): field ~a not found in this class (~a), Checking in parents...~%"
		  	field-name 
			instance)
		  (cond ((null (cadr (get-class-spec instance)))
			 (error 
			 	(format nil 
					"Field (class): This class has no parents so field ~a does not exist!~%"
					field-name)))
			(T (search-field-in-parents (cadr (get-class-spec instance)) field-name))))
		 (t (format *STANDARD-OUTPUT* 
  ...
  (when field-from-make
    ...
    (cond (
	   ...
	   (cond ((subtypep (type-of(cadr field-from-make))
		 ...
		 (t
		  (error
		   (format
		    nil
		    "Ffpof: field type ~a in make does not match its definition in its class (~a)~%"
		    (cadr field-from-make)
		    (caddr (find
			    (car field-from-make)
			    list-of-total-fields :test #'member)))))))
	  (t(error
	     (format
	     nil
	     "Ffpof: you have tried to initialize the same field more than once!~%"))))
...
(defun search-field-in-parents (parents field-name)
  (unless 
  	(field* (car parents) field-name)(search-field-in-parents (cdr parents) field-name)))
...
(defun is-method (class-name method-name)
	(cond
		(
			...
			(cond
				((or 
					...
					(error (format nil "Is-method: there is something wrong with this method")));;; FINISCI DI SCRIVERE
				(T 
					(format *STANDARD-OUTPUT* 
											...
											(cdr 
												(nth 3 
													(get-class-spec 
														(nth 1 class-name))))))))
							(format *STANDARD-OUTPUT* 
								"Is-method: method ~a not found in this class (~a). Checking in parents...~%" 
								method-name 
								(nth 1 class-name))
							(cond (	
								(null (cadr (get-class-spec (nth 1 class-name))))
									(error 
										(format nil 
											"Is-method: this class has no parents. Method ~a does not exist!~%"
											method-name)))
		...
		(T 
			(format *STANDARD-OUTPUT* 
			"Is-method (class): Checking ~a in class ~a~%"
			method-name 
			class-name)
			(cond (
					(not 
					...
					(cond (
								...
								(error 
									(format nil 
										"Is-method: Class has no parents. Method ~a does not exist!~%"
										method-name)))
								(T  
									(get-parents-method 
										(cadr 
...
(defun valid-field-type (field-type)
	(if 
		...
		(error 
			(format nil 
			"Valid-field-type: ~a is not a Common-Lisp type or an existing class" field-type))))
		...
		(and 
			...
			(not (subtypep 
					...
					(caddr (find 
								...
								:test #'member)))))					
		(error  (format nil 
						"Is-inherited: value ~a of field ~a is not a subtype of ~a"
...
(defun type-matching (current-field)
	(if (typep (cadr current-field) (caddr current-field))
		...
		(error
			(format nil 
				"Type-matching: value ~a of field ~a is not of the type specified (~a)"
				(cadr current-field) 
				(car current-field)
				(caddr current-field)))))
